...
export const AddHabitModal = ({ isOpen, onClose, onSubmit }: AddHabitModalProps) => {
  ...
  const [formData, setFormData] = useState<HabitFormData>(emptyForm);
  ...
  const handleSubmit = () => {
    ...
    const payload: CreateHabitRequest = {
      ...
      userId: 0, // backend assigns from session
    };

    if (effectiveUnit) {
      payload.unit = effectiveUnit;
      const num = Number(formData.targetValue);
      if (!isNaN(num) && formData.targetValue !== '') {
        payload.targetValue = num;
      }
    }
